...
class FoodProvider extends ChangeNotifier {
  ...
  StreamSubscription<List<CanteenModel>>? _canteenSub;
  ...
  void _listenToCanteens() {
    _canteenSub?.cancel();
    _canteenSub = _service.watchAllCanteens().listen((canteensData) {
      _canteens = canteensData;

      if (_canteens.isNotEmpty) {
        if (_featuredCanteen != null) {
          try {
            _featuredCanteen =
                _canteens.firstWhere((c) => c.id == _featuredCanteen!.id);
          } catch (_) {
            _featuredCanteen = _canteens.first;
          }
        } else {
          _featuredCanteen = _canteens.first;
        }
        ...
        if (_selectedCanteen != null) {
          try {
            _selectedCanteen =
                _canteens.firstWhere((c) => c.id == _selectedCanteen!.id);
          } catch (_) {
