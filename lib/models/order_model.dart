...
class OrderModel extends Equatable {
  ...
  int get etaMinutesRemaining {
    if (status == OrderStatus.ready ||
        status == OrderStatus.completed ||
        status == OrderStatus.cancelled) return 0;
    final prep = prepTimeMinutes ?? _estimatePrepTime();
    final elapsed = DateTime.now().difference(createdAt).inMinutes;
    final rem = prep - elapsed;
    return rem < 0 ? 0 : rem;
  }
  ...
  @override
  List<Object?> get props => [id, status, total, prepTimeMinutes];
}
